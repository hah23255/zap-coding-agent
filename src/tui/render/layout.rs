...
pub(super) fn draw_sidebar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if !app.quotas.is_empty() {
        let pct_color = |p: f32| -> Color {
            if p > 80.0 { Color::Rgb(220, 80, 80) }
            else if p > 60.0 { Color::Rgb(220, 180, 50) }
            else { Color::Rgb(80, 160, 255) }
        };
        let mut providers: Vec<&String> = app.quotas.keys().collect();
        providers.sort();
        for provider in providers {
            let entry = &app.quotas[provider];
            rows.push(Line::from(""));
            rows.push(Line::from(Span::styled(
                format!(" quota ({provider})"),
                Style::default().fg(head_c).bold(),
            )));
            if let Some(five_hour_pct) = entry.five_hour_pct {
                let reset_hint = entry.resets_at.as_deref()
            ...
            if let Some(seven_day_pct) = entry.seven_day_pct {
                rows.push(Line::from(vec![
